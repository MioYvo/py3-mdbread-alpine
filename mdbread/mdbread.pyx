...
def format_datetime(_datetime_bytes):
    if isinstance(_datetime_bytes, bytes):
        _datetime = _datetime_bytes.decode()
    else:
...
transformers = {
    "Long Integer": lambda x: int(x) if x != b"" else "",
    "Single": float,
...
cdef class MDB(object):
    ...
    def __init__(self, path):
        # mdb_init()
        self._handle = mdb_open(path.encode(), MDB_NOFLAGS)
